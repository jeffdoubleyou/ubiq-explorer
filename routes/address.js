...
router.get('/transactions/from/:addressId.:start.:end', function(req, res, next) {
  ...
  req.db.lrange('_shf_txn_from_' + addressId, start, end, function(error, result) {
        var resArray = [];
        for(var i in result) {
...
router.get('/transactions/from/total/:addressId', function(req, res, next) {
	...
	req.db.llen('_shf_txn_from_'+addressId, function(error, result) {
		var data = {
			error: error,
			transactions: result
		};
...
router.get('/transactions/to/:addressId.:start.:end', function(req, res, next) {
  ...
  req.db.lrange('_shf_txn_to_' + addressId, start, end, function(error, result) {
        var resArray = [];
        for(var i in result) {
            resArray.push(JSON.parse(result[i]));
        }
...
router.get('/transactions/to/total/:addressId', function(req, res, next) {
	...
	req.db.llen('_shf_txn_to_'+addressId, function(error, result) {
		var data = {
			error: error,
			transactions: result
		};
...
router.get('/transactions/balance/:addressId', function(req, res, next) {
  ...
  req.db.llen('_shf_balance_'+addressId, function(error, result) {
	...
	req.db.lrange('_shf_balance_'+addressId, start, end, function(error, result) {
		res.json(result);
	});
  });
...
router.get('/mined/:addressId.:start.:end', function(req, res, next) {
  ...
  if(addressId.substring(0,2) != '0x') {
    ...
    req.db.lrange('_shf_block_miner_'+ addressId, start, end, function(error, result) {
        var resArray = [];
        for(var i in result) {
            resArray.push(JSON.parse(result[i]));
        }
...
router.get('/mined/blocks/:addressId', function(req, res, next) {
	...
	req.db.llen('_shf_block_miner_'+addressId, function(error, result) {
		var data = {
			error: error,
			blocks: result
		};
...
router.get('/minechart/:addressId', function(req, res, next) {
	...
	req.db.lrange('_shf_block_miner_'+addressId,0,1000, function(error, result) {
